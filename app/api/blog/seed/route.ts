...
import { seedPosts } from "@/app/lib/db/blog";

// ─── POST /api/blog/seed — Seed database with mock posts ────────────────────
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    `,
      ...
      {
        title: "How We Delivered a Campus Management System in 12 Weeks",
    ...
    const insertedCount = await seedPosts(SEED_POSTS);

    return NextResponse.json({
      success: true,
      message: `Seeded ${insertedCount} new posts (${SEED_POSTS.length - insertedCount} already existed)`,
      insertedCount,
      totalProvided: SEED_POSTS.length,
    });
  } catch (error) {
    console.error("POST /api/blog/seed error:", error);
    return NextResponse.json(
